        ...
        public List<string> ExtractArchives(string sourceArchivePath, string targetDirectory, IProgress<ArchiveProgressInfo>? progress = null)
        {
            ...
            while (archivesToProcess.Count > 0)
            {
                ...
                foreach (var file in Directory.GetFiles(currentExtractDir, "*.*", SearchOption.AllDirectories))
                {
                    if (IsLogFile(file))
                    {
                        //  ДОБАВЬ УНИКАЛЬНОЕ ИМЯ
                        string archiveName = Path.GetFileNameWithoutExtension(sourceArchivePath);
                        string uniqueFileName = $"{archiveName}_{Path.GetFileName(file)}";
                        string uniquePath = Path.Combine(currentExtractDir, uniqueFileName);
                        ...
                        extractedLogFiles.Add(uniquePath);
                    }
